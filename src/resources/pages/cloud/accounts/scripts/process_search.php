    ...
    function process_search()
    {
        if(isset($_POST['by']) == false)
        {
            Actions::redirect(DynamicalWeb::getRoute(
                'cloud/accounts', array('callback' => '100')
            ));
        }
        ...
        try
        {
            ...
            Actions::redirect(DynamicalWeb::getRoute(
                'cloud/manage_account', array('id' => $Account->ID)
            ));
        }
        catch(InvalidSearchMethodException $invalidSearchMethodException)
        {
            Actions::redirect(DynamicalWeb::getRoute(
                'cloud/accounts', array('callback' => '103')
            ));
        }
        catch(AccountNotFoundException $accountNotFoundException)
        {
            Actions::redirect(DynamicalWeb::getRoute(
                'cloud/accounts', array('callback' => '101')
            ));
        }
        catch(Exception $exception)
        {
            Actions::redirect(DynamicalWeb::getRoute(
                'cloud/accounts', array('callback' => '102')
            ));
        }
    }